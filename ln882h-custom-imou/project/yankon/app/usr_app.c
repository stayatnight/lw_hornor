...
wifi_softap_cfg_t ap_cfg = {.ssid    = "LN_AP_8899",
                            .pwd     = "12345678",
                            .bssid   = mac_addr,
                            .ext_cfg = {
                                .channel         = 6,
...
int8_t rlLampBriCtrlNextClass(uint32_t ulPeroidMs)
{
    ...
    percent =
        rlLampGetNextClassVal(ucBriDirection, percent, k_a_uwBriPercent, sizeof(k_a_uwBriPercent) / sizeof(uint32_t));
...
static void wifi_init_ap(void)
{
    ...
    if ((strlen(ap_cfg.pwd) != 0) && (ap_cfg.ext_cfg.authmode != WIFI_AUTH_OPEN) &&
...
static int _rlTaskKeyInit(void)
{
    ...
    myKeyboardRigisterCallback(_normalKeyShortPressCb, _normalKeyShortReleaseCb, _normalKeyLongPressCb,
...
#if (APP_DEV_TYPE_USED == APP_DEV_TYPE_LAMP_NIGHT || APP_DEV_TYPE_USED == APP_DEV_TYPE_LAMP_NIGHT_PTJX)
    ulPwm[0] = (uwCCT == LIGHT_COLOR_CCT_MIN) ? LIGHT_PWM_MAX
                                              : ((uwCCT == LIGHT_COLOR_CCT_MAX) ? LIGHT_PWM_MIN : LIGHT_PWM_MAX);
    ulPwm[1] = (uwCCT == LIGHT_COLOR_CCT_MIN) ? LIGHT_PWM_MIN
                                              : ((uwCCT == LIGHT_COLOR_CCT_MAX) ? LIGHT_PWM_MAX : LIGHT_PWM_MAX);
#else
    tmpPwm   = LIGHT_PWM_MIN + (uint32_t)((LIGHT_PWM_MAX - LIGHT_PWM_MIN) * pwmBri * afPWM[0] / LIGHT_BRIGHT_MAX);
    ulPwm[0] = APP_RANGE(tmpPwm, LIGHT_PWM_MIN, LIGHT_PWM_MAX);
    tmpPwm   = LIGHT_PWM_MIN + (uint32_t)((LIGHT_PWM_MAX - LIGHT_PWM_MIN) * pwmBri * afPWM[1] / LIGHT_BRIGHT_MAX);
    ulPwm[1] = APP_RANGE(tmpPwm, LIGHT_PWM_MIN, LIGHT_PWM_MAX);
#endif
    ulPwm[2] = 0;
    ulPwm[3] = 0;
    // my_hal_log_debug("bri %d pwmbri %d cct %d to pwm %d %d\r\n", pLampParam->uwBri, pwmBri, uwCCT, ulPwm[0],
    // ulPwm[1]);
...
static int rlTaskLampInit(void *arg)
{
    ...
    myLampCreateLamp(&gucLampId, pLampParam, _lampPwmOutput, _lampColor2Pwm, getMyDimmingCurve(LIGHT_PWM_CURVE), NULL,
                     ln_chip_reboot);
    // 30s后关闭长按重置
...
void creat_usr_app_task(void)
{
    ...
    if (OS_OK != OS_ThreadCreate(&g_usr_app_thread, "WifiUsrAPP", usr_app_task_entry, NULL, OS_PRIORITY_BELOW_NORMAL,
...
#if WIFI_TEMP_CALIBRATE
    if (OS_OK != OS_ThreadCreate(&g_temp_cal_thread, "TempAPP", temp_cal_app_task_entry, NULL, OS_PRIORITY_BELOW_NORMAL,
...
#if LAMP_TASK_EN && LAMP_TASK_EN == 1
    if (OS_OK != OS_ThreadCreate(&g_lamp_thread, "LampApp", usr_app_light_task_entry, NULL, OS_PRIORITY_BELOW_NORMAL,
...
#if KEY_TASK_EN && KEY_TASK_EN == 1
    if (OS_OK != OS_ThreadCreate(&g_key_thread, "KeyApp", key_app_task_entry, NULL, OS_PRIORITY_BELOW_NORMAL,
                                 KEY_TASK_STACK_SIZE)) {
    }
#endif
    ...
    /* print sdk version */
    {
        LOG(LOG_LVL_INFO, "LN882H SDK Ver: %s [build time:%s][0x%08x]\r\n", LN882H_SDK_VERSION_STRING,
            LN882H_SDK_BUILD_DATE_TIME, LN882H_SDK_VERSION);
    }
}
